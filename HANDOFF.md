...
마지막 업데이트: **2026-08-17 (H)** (브루마블 대수술 — 카드/기금/순서주사위/총자산 + 가짜 화가 커서)

---

### 🗓️ 2026-08-17 세션 (H) — 🎲 브루마블 대수술 + 가짜 화가 커서

이 날 커밋 11개(HANDOFF 제외 10개). **오전**: `b17e0b6`·`3a52b14`·`c40f901`·`166d73c` (실플레이 피드백 반영). **오후**: `4e04015`·`8f78e3b`·`857a25f`·`7adbbd4`·`dfe385a`·`a6aa74e`·`d8c7c8a`·`afc5235`.
사실상 브루마블 하나만 하루 종일 판 날. 아래는 주제별로 묶었고, 시간순은 커밋 해시 순서를 따를 것.

**🖱️ 가짜 화가 캔버스 커서 안 보임 수정** (`fake-artist/index.ts`+`render.ts`. ⚠️ `git add -A` 로 브루마블 커밋 `4e04015` 에 딸려 들어갔음 — 커밋 메시지엔 안 적혀 있으니 나중에 찾을 때 주의)
- **원인**: 공용 `theme.css .game-canvas{cursor:none}` — 에어하키 말렛용으로 넣은 전역 규칙인데, 가짜 화가가 안 덮어써서 그림판 위에서 마우스가 통째로 사라졌음. draw-quiz·story-draw 는 마운트 때 `canvas.style.cursor` 를 직접 세팅해서 안 걸렸던 것.
- **수정**: `updateCanvasCursor()` — 내 차례엔 **내 펜 색 동그라미**(크기 = `PEN_WIDTH × scale` 로 실제 획 굵기와 일치, 어두운 획 위에서도 보이게 흰 외곽링), 아니면 기본 화살표. data URI 를 매 프레임 만들지 않게 `cursorKey`(색+배율) 캐시. renderer 에 `getScale()` 추가. `destroy` 시 `''` 로 되돌려 다음 게임에 안 샘.
- **교훈**: 캔버스 쓰는 새 게임은 `.game-canvas` 의 `cursor:none` 을 반드시 덮어쓸 것.

**🎲 브루마블 — 오전 4커밋 (실플레이 피드백)**
- `b17e0b6` 세계여행으로 출발선 넘어도 월급 안 붙던 것 / 월급 팝업 `playFx()` 인수 불일치 TypeError / 비용 부족 시 조용히 턴 넘어가던 것을 info 안내로. **건물비 30% 인하**, 올림픽 배수 `min(5,n+1)` → `min(32,n*2)`, 세계여행 40만→3만. 모달·토스트를 화면 중앙 → **게임판 중앙**(`mountScrim`, 딤도 판 안쪽만 → 우측 패널 계속 보임). 보너스 게임 관전자도 실시간 표시.
- `3a52b14` 더블 3연속 무인도인데 말이 원래 칸에 남던 것(`toDesert` 가 `s.pos` 를 안 옮김) / 3번째 더블 주사위·이동 연출이 아예 안 나오던 것 / **자금 마련 창의 파산 버튼이 `bankrupt()` 를 직접 불러 남은 현금이 소멸**하던 것 / **`state.log` 가 어디서도 안 그려지던 것**(판 중앙에 한 줄 추가 — '더블! 한 번 더' 같은 안내가 전부 안 보이고 있었음). `noExtraRoll` 신설(무인도 탈출 더블·이동 카드·세계여행권 획득은 더블이어도 추가 굴림 없음).
- `c40f901` **통행료 면제권을 실제로 쓸 수 있게** — 예전엔 주사위 굴리기 전에 미리 켜둬야만 작동해서 사실상 못 쓰는 카드였음. `Pending.tollAsk` 신설로 통행료가 뜨는 순간 물어본다. 통행료 정산을 `settleToll()` 로 분리. **랜드마크 선행조건 구멍** — 한 창에서 3개 체크하는 순간 랜드마크까지 열려 땅 구매~랜드마크가 한 턴에 끝났음(클라·호스트 양쪽 수정, 창 진입 시점 상태로 검사).
- `166d73c` **모든 이동을 판 경로 따라 한 칸씩 걷게** 통일 — 카드 이동(출발로/최고가/무인도 유배)이 직선으로 날아가던 것 제거. 뒤로 3칸은 `cardFx.back` 으로 역방향. `startMoveSeq` 의 `gap>12 순간이동 예외` 제거(더블 3연속 무인도행이 여기 걸려 말이 안 움직이는 것처럼 보였음). 칸 수 많아도 총 시간 일정하게 칸당 시간 조절.

**🎴 브루마블 — 황금열쇠 20종 확장 + 현금 증감 표시** (커밋 `4e04015`)
- **🆕 사회복지기금 카드**(4%) — 벌금(병원비·속도위반)과 세금(재산세·국세청)으로 낸 돈이 `state.fund` 에 쌓이고 이 카드로 **전액 수령**. 기금 0원이면 안내만 뜨고 넘어감.
  - `pay()` 에 **`toFund` 인수 추가** — 예전엔 은행행(`to=null`)이면 전부 fund 에 들어가서 세계여행비·무인도 탈출비 같은 "서비스 값"까지 섞여 있었음. 이제 벌금·세금만.
  - 모인 금액은 **우측 패널 맨 위 pill** 로 상시 표시(늘어날 때 bump, 0원이면 회색).
- **🆕 서울로 카드**(3%).
- **최고가 도시로 = 동적** — `TOP_CITY_TILE` 고정 상수(항상 서울)에서 **`topTollTile(state, peer)`** 로. 지금 통행료가 가장 비싼 칸을 매번 계산(건물 단계·컬러 독점·올림픽·섬/해변 배수 전부 반영). `tollBreakdown` 이 내 땅·빈 땅·정전 칸을 0으로 주므로 자연히 제외. **서 있는 칸도 제외**(제자리 이동으로 통행료 2중 부과 방지). 남의 땅이 없으면 정가 최고(서울) 폴백.
- **확률 재조정 — weight 합을 100 으로 맞춰서 이제 weight = %** (`CARDS` 주석에 명시. 조정 시 합 100 유지할 것)

  | 분류 | 이전 | 지금 | 내역 |
  |---|---|---|---|
  | 돈 획득 | 29.3% | **24%** | 은행이자 13→11 · 보너스마블 8→7 · 생일 6→4 |
  | 돈 손실 | 24.2% | **21%** | 병원비 9→8 · 속도위반 9→8 · 재산세 6→5 |
  | 공격 | 9.1% | **14%** | 교환 3→4 · 지진 3→5 · 정전 3→5 |
  | 이동 | 22.2% | 23% | **무인도 4 ↔ 출발 6 맞교환**, 서울로 3 신규, 뒤로3 5→4, 세계여행 4→3 |
  | 보관형 | 10.1% | 9% | 면제권 6→5 |
  | 올림픽 | 5.1% | 5% | — |
  | 기금 | — | **4%** | 신규 |

- **💵 현금 증감 뱃지** — 플레이어 행 옆에 `+₩150,000`(초록) / `−₩120,000`(빨강). 팝 → 약 1.2초 머무름 → 위로 흐르며 소멸(총 2.2초, reduced-motion 이면 페이드만).
  - 호스트가 **`afterChange` 에서 직전 스냅샷과 diff** 해 `moneyFx` 로 실어보냄 → 통행료·카드·세금·월급·구매·건설·판매 **경로를 안 가리고 전부 잡힘**. 돈 움직이는 곳마다 연출 코드 심을 필요 없음.
  - 행 HTML 은 매 렌더 통째로 재생성되므로 뱃지는 **별도 오버레이(`#bm-mfx`)** 에 얹고 행 `offsetTop` 만 따라감(안 그러면 애니가 매 프레임 리셋).
- **정리** — 죽은 콘서트 칸 제거(BOARD 에 없는데 타입·핸들러만 남아 있었음). 코너 `kind:'welfare'` → **`'olympic'`** 리네임(실제로 올림픽 코너인데 옛 이름이라 신규 기금 카드와 헷갈림). ⚠️ 이때 `render.ts` 의 코너 **일러스트 매핑에도 같은 키가 있어서** 같이 안 고쳤으면 올림픽 코너 그림이 사라질 뻔했음 — 리네임 시 `tileIcon`/`tileIll` 둘 다 확인.

**🎲 브루마블 — 시작 순서 주사위 + 현금/총자산 + 벌금 땅팔기** (커밋 `8f78e3b`)
- **🆕 시작 순서 정하기** — `phase` 에 **`'order'`** 추가. 게임 들어오면 전원이 주사위 2개를 굴려 **높은 순으로 순서 결정**.
  - **동점 = 그 사람들끼리만 재굴림**. 굴림을 `orderRolls[peer]: number[]` 로 쌓고 **앞에서부터 사전식 내림차순 비교** → "1차가 높으면 무조건 앞, 1차가 같을 때만 2차로 가른다"가 자연히 나옴(**2차에 12를 굴려도 1차에서 앞선 사람은 못 넘음**). `ORDER_MAX_ROUNDS=5` 넘으면 좌석 순.
  - **15초 백스톱** — 안 굴리는 사람은 호스트가 대신 굴림(창 닫아둔 한 명 때문에 방이 멈추지 않게). `orderTimer` 500ms 인터벌, phase 넘어가면 자동 해제.
  - 더미는 즉시 자동 굴림. 순서 확정 후 2.6초 결과 보여주고 시작. 순서 정하기 중 이탈하면 대기에서 제외.
  - ⚠️ `s.order` 가 재정렬되므로 tiebreak 기준용 **`seatOrder`(원래 좌석 순)** 를 모듈 필드로 따로 들고 있음.
- **💰 용어 3개로 통일** — **현금**(손에 든 돈) · **부동산**(다 팔면 들어오는 돈) · **총자산**(둘의 합). `rules.ts` 에 `estateValue()`/`totalAssets()`.
  - 우측 패널 각 행 = 현금(굵게) + `총 ₩N`(작게) 2줄, 헤더에 범례.
  - **결과 화면 순위가 현금만 보고 있던 것 → 총자산 기준으로 수정.** 땅에 다 박은 사람이 억울하게 밀리고 있었음. summary 에 `estate`/`assets` 추가(구버전 호환 폴백 있음), 내 결과에 현금·부동산 내역 표시.
- **🏷️ 병원비·속도위반·생일 축하도 땅 팔 기회** — 현금 부족 시 바로 파산하던 것을 통행료·세금과 동일하게 자금 마련 창 경유로.
  - 생일 축하는 **차례가 아닌 사람들이** 내야 해서 **`debtQueue`(순차 처리) + `Pending.raiseFunds.debtor`** 도입. `hostHandle` 이 "차례인 사람만" 받던 것을 **채무자의 `sell`/`payDebt`/`giveUp` 은 예외 허용**, 반대로 **남이 빚 갚는 중엔 차례 주인도 못 끼어들게** 막음.
  - **규칙 변경**: 생일 축하가 "가진 만큼만 내고 끝"에서 **땅을 팔아서라도 전액 낸다(못 내면 파산)** 로 바뀜.
  - 이탈 처리 — 채무자가 나가면 pending 풀고 큐 이어감, 차례 주인이 나가면 그 턴 빚은 통째로 폐기.
- **💀 다 팔아도 안 되면 파산 버튼만** — `총자산 < 청구액` 이면 땅 목록·지불 버튼을 아예 숨기고 "전부 팔아도 ₩N 부족해요" + 파산하기 버튼만. 팔아봐야 소용없는 목록을 훑게 하지 않음.
- **검증**: 빌드 통과 + `node --experimental-strip-types` 로 순수 로직 단위 테스트(동점 정렬·재굴림이 그룹 안에서만 갈리는지·3명 동점 후 2명 재동점·총자산·weight 합 100·`topTollTile` 5케이스) 전부 통과. **브라우저 실플레이는 미검증.**

**🖥️ UI 잔버그 2건** (커밋 `857a25f`)
- **채팅 전송 버튼이 패널 밖으로 잘림** — `.chat-input` 이 `flex:1` 인데 flex 아이템 기본 `min-width:auto` 라 input 고유 폭(`size=20`, 약 208px) 밑으로 안 줄어든다. 패널 280px 에서 패딩 24 + gap 8 을 빼면 남는 자리가 딱 그만큼이라, 입력창이 자리를 안 내주고 40px 원형 버튼이 밀려나 `.chat-panel{overflow:hidden}` 에 잘렸음. **`min-width:0` + `flex:1 1 0`** 으로 수정, 같은 패턴이던 werewolf `.ww-chat-input` 도 함께. → **교훈: flex 자식 input/텍스트는 `min-width:0` 을 습관적으로 붙일 것.**
- **순서 정하기 주사위 눈 2개 + 굴림 애니** — 합계 숫자만 보여줘서 "주사위 하나인데 왜 두 자리냐"로 보였음. `BMState.orderDice` 추가(순위는 기존 `orderRolls` 합계 그대로, 눈은 표시 전용 index 1:1). 재굴림 이력은 앞에 작은 합계, 마지막 굴림만 주사위 2개+합계. 굴린 사람 것만 0.62초 스핀 후 착지.

**💵 출발선 통과 월급 누락 + 팝업 유실** (커밋 `7adbbd4`) — Henry 리포트 "세계여행 갔는데 월급 안 들어옴". 원인이 **두 개**였다.
- **실제 돈 누락**: `cardMove()` 가 `s.pos` 만 갈아끼우고 출발선 통과를 안 봤음. `166d73c` 로 카드 이동이 판 경로를 걷게 된 뒤로는 말이 출발 칸을 지나는 게 눈에 보이는데 월급이 안 붙었다. → `dest < from` 이면 월급·바퀴 인정(`dest===0` 은 `resolveLanding` 이 주므로 바퀴만, `back`=뒤로3칸은 역주행이라 없음). **세계여행 카드·최고가 도시로**가 영향받았음.
- **팝업 유실**: 세계여행 도착 처리에서 `s.fx` 에 월급 gain 을 넣어도 바로 뒤 `resolveLanding` 이 통행료 fx 로 덮어썼다. 그 사이 렌더러는 비행 애니로 `busy` 라 월급 fx 를 소비하지 못하고 그냥 사라짐. → **교훈: `s.fx` 는 슬롯이 하나뿐이라 같은 틱에 두 번 쓰면 앞엣것이 사라진다.**

**🌏 세계여행 선택제 + 치트 + 금액 표시 정리** (커밋 `dfe385a`, `a6aa74e`)
- **세계여행 = 선택제** — 밟자마자 3만원 빠지던 것을 없애고, **다음 내 턴이 시작될 때** 갈지 말지 물어본다(`Pending.travelOffer`, `advanceTurn` 에서 세팅). 가면 비용 내고 목적지 선택 = 이번 턴의 이동, 안 가면 평소처럼 주사위. 현금 부족하면 '간다' 비활성. 세계여행 **카드**로 받은 무료 여행권(`travelReady`)은 그대로 비용 없이 바로 목적지 선택. 밟았을 때 아무것도 안 주므로 `noExtraRoll` 도 안 걸어 더블 유지.
- **판 위 금액 팝업 전부 제거** — 월급·통행료·획득의 `+₩`/`−₩` 큰 숫자를 다 뺐다(Henry 요청). 금액은 **우측 패널 현금 증감 뱃지 한 곳**에서만. 판에 남는 건 통행료 흔들림·동전·파산 문구뿐. ※ 한 턴에 월급+통행료면 뱃지는 **합쳐진 순액**으로 뜬다(분리하려면 별도 작업 필요).
- **증감 뱃지를 패널 카드 바깥 오른쪽으로** — 현금 숫자를 덮고 있었음.
- **못 사는 걸 전원이 보게** — info 안내에 닉네임·필요 금액·현재 현금("○○님 현금 부족 — ₩N 필요"). 자동 닫힘 1초 → 1.9초(`INFO_MS`).
- **황금열쇠 확률** — 돈 손실 21 → **18**(병원비 8→7·속도위반 8→7·재산세 5→4), 줄인 3 을 사회복지기금 4 → **7** 로 이전. 합계 100 유지.
- **치트 `/showmethemoney`** — 현금 +20만. 채팅 전송·기록 없음, **증감 뱃지도 안 뜬다**(적용 시 `moneySnap` 도 같이 올려 diff 가 0 이 되게). `GameModule.onCheatCode?(code)` 훅 + `types.ts CHEAT_CODES`.

**🧯 세계여행 선택창이 안 눌리던 것** (커밋 `d8c7c8a`) — `travelOfferModal` 이 이 파일의 모달 규약 ①`closeModal()` ②`mountScrim` 후 `openKind` 세팅 을 둘 다 빼먹었다. ①때문에 직전 모달("상대방이 건설 중" 대기창)이 위에 남아 클릭을 다 먹었고, ②때문에 매 프레임 새로 만들어지고 있었음. **`.bm-btns button:disabled` 스타일도 없어서** 잠긴 '간다'(와 자금 마련의 '지불')가 멀쩡한 버튼처럼 보였던 것도 같이 수정. → **교훈: 이 파일에 모달 추가할 땐 `closeModal()` → `mountScrim()` → `openKind = key` 3단 규약 반드시.**

**🧹 데드코드 정리 + 파산 연출 부활** (커밋 `afc5235`)
- **파산 연출 부활** — `playFx` 에 `kind:'bankrupt'`(화면 흔들림 + "○○ 파산!") 분기가 있는데 `s.fx` 에 넣는 곳이 없어 죽어 있었다. `bankrupt()` 에서 세팅.
- **뱃지 좁은 화면 잘림** — `@media(max-width:820px)` 세로 레이아웃에선 패널이 100% 폭이라 카드 바깥(`left:100%`) 뱃지가 화면 밖으로 나갔음. 그 폭에선 안쪽 정렬로 되돌림.
- **대기실 치트 노출** — 치트 가로채기가 gameScreen 에만 있어서 대기실 채팅에선 코드가 그냥 전송됐다. **`wireChatPanel`(chat.ts) 한 곳으로 이동**(`ChatCallbacks.onCheat?`) → 대기실·게임·결과 어디서 쳐도 채팅엔 안 남고, 처리할 게임이 없으면 조용히 삼킴.
- 제거: `tollFor()`(호출 0회) · `TileType`(사용 0회) · CSS `.bm-tokfly`(`166d73c` 에서 엘리먼트 제거 후 잔존) · CSS `.bm-fxmul`/`.bm-fxnum.gain`(금액 팝업 제거로 무용).
- **`BMPlayer.tollExempt` 일체 제거** — 보관함 면제권 버튼이 항상 `disabled` 라 `useHeld('tollExempt')` 호출 불가 → 필드가 영원히 false → `resolveLanding` 의 `if (p.tollExempt)` 분기도 도달 불가. `c40f901` 의 `tollAsk` 모달로 대체된 잔재였음(면제권 카드 자체는 정상 동작).
- 정리 후 `.bm-*` 클래스 126개 재스캔 → 미사용 0건.

**⚠️ 이 세션 미검증(다음에 할 것)**
- 브루마블 **순서 정하기 실멀티**(3인+ 동점·백스톱·이탈) — 주사위 눈/스핀 표시 포함
- 브루마블 **생일 축하로 여러 명이 연달아 땅 파는 흐름**(차례 아닌 사람 모달·순차 진행)
- 브루마블 **세계여행 선택제**(간다/안 간다 양쪽, 돈 부족 시 잠금, 카드 무료 여행권)
- 브루마블 사회복지기금 적립/수령, 서울로·최고가 카드 이동, 출발선 통과 월급, 파산 연출
- 현금 증감 뱃지 — 위치/타이밍, 좁은 화면(<=820px) 세로 레이아웃
- 치트 `/showmethemoney` — 게임 중·대기실 양쪽에서 채팅에 안 남는지
- 가짜 화가 커서(내 차례 색 동그라미 ↔ 화살표 전환)
- 오전 4커밋(걷기 연출 역방향·tollAsk 모달·판 중앙 모달/로그) 실플레이

**🧹 브루마블 남은 것 (안 건드림)**
- 파산한 사람 땅이 무주공산이 됨 (원래 룰은 채권자가 인수)
- 한 턴에 월급+통행료가 겹치면 증감 뱃지가 순액으로만 보임 (분리하려면 fx 큐 필요)
- 재산세가 **현금 10%** 라 현금을 쥐고 있으면 유독 아픔 — 상한(예: 30만) 검토 여지
...
- **🎁 황금열쇠/포춘카드**: 가중치 뽑기 19종. *(→ 2026-08-17 세션 H 에서 **20종·weight 합 100** 으로 재편. 최신 목록·확률은 그쪽 참고)* 즉발(은행이자·보너스·**복권 +150만 희귀 JACKPOT**·병원비·과속·재산세10%·기금독식·출발이동·무인도유배·뒤로3칸·최고가도시) + 보관형(올림픽개최권·통행료면제권·무인도탈출권·세계여행권, 자동보관·자유사용) + **공격형(도시교환·지진=건물1단계파괴·정전=통행료3턴0)**. 각 카드 전용 연출(말날기·파편·스왑글로우·동전샤워·토스트) + reduced-motion 대응.
...
**다음 작업** (우선순위):
- 🔥 **브루마블 실제 플레이 검증** — 2026-08-17 세션 (H) 에서 하루 종일 손봤는데 **전부 미검증**. 우선순위 높은 것부터:
  1. **순서 정하기**(3인+ 동점·15초 백스톱·도중 이탈·주사위 눈/스핀)
  2. **생일 축하로 여러 명이 연달아 땅 파는 흐름**(차례 아닌 사람에게 모달이 뜨는지, 순차 진행, 중간 이탈)
  3. **세계여행 선택제**(간다/안 간다, 돈 부족 시 '간다' 잠금, 카드 무료 여행권) + 출발선 통과 월급
  4. 사회복지기금 적립/수령, 서울로·최고가 카드 이동, 파산 연출, 현금 증감 뱃지(위치·타이밍·좁은 화면), "다 팔아도 부족" 파산 전용 창
  5. 치트 `/showmethemoney` 가 게임 중·대기실 양쪽에서 채팅에 안 남는지
  6. 오전 4커밋(걷기 연출 역방향·tollAsk 모달·판 중앙 모달/로그·랜드마크 선행조건)
  7. 기존 미검증분 — 통행료 배수/독점승/파산순서, 카드 20종 효과·연출, 세금 이벤트창 동기화, 올림픽/보너스 흐름, 밸런스(금액·확률·건물비)
- 🖱️ **가짜 화가 커서** 실플레이 확인(내 차례 = 내 색 동그라미 ↔ 아니면 화살표).
