...
- **🐛→✅ 버그 "확인 다 눌러도 안 넘어감"(5인 실플레이)** — 원인: 한 명의 `av:ready`가 호스트에 유실 → readySet 안 참 → deal 영구 대기(60초 백스톱 전까지). 재전송 로직이 없던 게 문제(커밋 `5a91a79`). **수정**: 게스트 loop 에서 같은 페이즈에 머물면 내 입력(확인/선발/투표/원정카드/암살)을 2.5초마다 재전송(호스트 핸들러 전부 멱등). ⚠️ **werewolf 도 동일 잠재 버그 있음 — 미수정**.
- **⚠️ 남은 검증**: 위 재전송 수정의 5인 실멀티 재확인, 타겟 시크릿 채널·공개투표·비밀 원정카드·암살, 이탈 시 진행(리더/암살자/원정대원).
