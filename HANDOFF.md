...
이 날 커밋 6개. 앞의 4개(`b17e0b6`·`3a52b14`·`c40f901`·`166d73c`)는 오전 세션(실플레이 피드백 반영), 뒤의 2개(`4e04015`·`8f78e3b`)는 오후 세션.
...
**⚠️ 이 세션 미검증(다음에 할 것)**
- 브루마블 **순서 정하기 실멀티**(3인+ 동점·백스톱·이탈)
- 브루마블 **생일 축하로 여러 명이 연달아 땅 파는 흐름**(차례 아닌 사람 모달·순차 진행)
- 브루마블 사회복지기금 적립/수령, 서울로·최고가 카드 이동, 현금 증감 뱃지 타이밍
- 가짜 화가 커서(내 차례 색 동그라미 ↔ 화살표 전환)
- 오전 4커밋(걷기 연출 역방향·tollAsk 모달·판 중앙 모달/로그) 실플레이

**🧹 브루마블 남은 것 (안 건드림)**
- 파산한 사람 땅이 무주공산이 됨 (원래 룰은 채권자가 인수)
- `fx.kind='bankrupt'` 연출이 세팅되는 곳이 없어 죽어 있음
- `BMPlayer.tollExempt` — 보관함 미리켜기 버튼을 없앤 뒤로 UI 에서 켤 방법이 없는 사실상 죽은 필드
...
**다음 작업** (우선순위):
- 🔥 **브루마블 실제 플레이 검증** — 2026-08-17 세션 (H) 에서 대폭 손봤는데 **전부 미검증**. 우선순위 높은 것부터:
  1. **순서 정하기**(3인+ 동점·15초 백스톱·도중 이탈)
  2. **생일 축하로 여러 명이 연달아 땅 파는 흐름**(차례 아닌 사람에게 모달이 뜨는지, 순차 진행, 중간 이탈)
  3. 사회복지기금 적립/수령, 서울로·최고가 카드 이동, 현금 증감 뱃지 타이밍, "다 팔아도 부족" 파산 전용 창
  4. 오전 4커밋(걷기 연출 역방향·tollAsk 모달·판 중앙 모달/로그·랜드마크 선행조건)
  5. 기존 미검증분 — 통행료 배수/독점승/파산순서, 카드 20종 효과·연출, 세금 이벤트창 동기화, 세계여행/올림픽/보너스 흐름, 밸런스(금액·확률·건물비)
