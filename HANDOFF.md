...
- **✅ 검증(2026-07-29 Playwright)**: 솔로(봇4) 및 실제 2클라(호스트+게스트) 모두 deal→team→vote 전환 정상, 콘솔 에러 0. 사람이 리더일 때 원정대 선발 UI(칩 선택 0/2→2/2→확정→투표)도 정상 동작 확인.
- **⚠️ Henry 리포트 "확인 다 눌러도 안 넘어감"** → 하드버그 재현 실패(위 모든 경로 정상). 유력 원인: 실제 사람 게임에선 첫 리더가 사람 → **그 리더가 원정대를 뽑기 전까지 다른 사람은 "리더 X가 고르는 중" 대기**(봇은 즉시 뽑아 안 보였음). 즉 "안 넘어간 것처럼" 보이는 UX. → **후속: 리더 차례 강조 UX 개선 검토**(누구 차례인지/리더는 뽑아야 함 명확히). 또는 확인 메시지 유실 시 60초 자동진행(DEAL_TIMEOUT)이 너무 길어 그리 보일 수도.
- **⚠️ 남은 검증**: 5인+ 실멀티(타겟 시크릿 채널·공개투표·비밀 원정카드·암살), 이탈 시 진행(리더/암살자/원정대원).

---

### 🗓️ 2026-07-21 세션 (E) — 늑대인간 정식 룰 대확장 + 브루마블/포트리스 실플레이 수정

**🐺 한밤의 늑대인간 — 정식 룰(ONUW) 전면 확장** (커밋 6de5343 외 여러 개)
- **역할 12종**: 늑대·**하수인(minion)**·**메이슨(mason)**·예언자·강도·말썽쟁이·주정뱅이·수면증(불면증 개명)·**사냥꾼(hunter)**·**탄넬러(tanner)**·마을주민·**도플갱어(doppelganger)**. 신규 5종 일러스트(`ROLE_SVG`).
- **밤 순서 정식화**: 도플1·늑대2·하수인3·메이슨4·예언자5·강도6·말썽7·주정8·수면9 (`ROLE_META.nightOrder`).
- **승패 재작성**(`computeWin`/`resolveHunterDeaths`): 탄넬러 처형=단독승(최우선) / 사냥꾼 처형 시 지목자 연쇄처형 / 늑대 생존=늑대팀승(하수인 포함) / 최종 카드 기준, `winners` 목록.
- **도플갱어 완전 구현**: 밤 1번째 복사 → curCard 가 복사역할이 됨(승패도 그 팀) → 복사 직업 능력 즉시 수행(강도=실제 강도질, 예언자=확인 등). 알려진 단순화: 도플-늑대는 늑대 일방 확인, 도플-수면증은 복사 시점 카드.
- **두 모드**: **일반**(인원별 공식조합 3~10인, `SETUPS`) / **랜덤**(방옵션 `mode=free` → `setup` 페이즈에서 방장이 직업 피커로 선택, 늑대만 다중·나머지 1장, `validateFreeSetup` 카드수/경고). **최대 10인**.
- **결과 공개 확장**: 처음/최종/가운데/**밤 행동 로그**/**카드 교환 로그**/투표/승리팀(4종)/승리자.
- **버그 수정(실플레이)**: (1) deal 렌더키에 myOrigRole/readyCount 누락 → 확인버튼 지연·"기다리는중" 카운트 정지·시작 지연 해결. (2) 결과화면 매프레임 재생성 → "다음" 버튼 클릭 안 먹음 → `resultKey` 가드(9초 자동이동 제거, 각자 다음 버튼). (3) 예언자 확인화면에 본 카드 크게 표시. (4) 내 역할카드가 밤에 바뀐 현재 카드 반영(강도/수면증/도플갱어). (5) 채팅 제한 200→500(werewolf+플랫폼).

**🎲 브루마블 실플레이 수정**
- **인수 비용** ×2 → **×1.5** + 건물비 인하로 이중 인하.
- **올림픽 단일 개최지** — 새 도시 열면 기존 해제(같은 곳 재개최는 배수 누적). 개최 모달에 건너뛰기 안내.
- **자기 땅 판매** — 내 차례 "🏷️ 내 땅 팔기" 버튼(자발적) + 통행료/세금 부족 시 **파산 대신 raiseFunds 페이즈**(땅 팔아 마련, 지불/파산). 환급=땅값+건물비 전액. 더미도 대응.
- **건물 건설비 라인별 ~35% 인하**(`LINE_BUILD`).
- **말 가시성** — 크기↑ + 흰 후광 2겹 + 그림자 / 내 말은 더 크게+통통 애니(reduced-motion 정지).

**🏰 포트리스** — 강하게 쏜 유도탄이 화면 밖(x)으로 나가 컬링돼 사라지던 것 → 유도탄은 y>700(화면 한참 아래)일 때만 미스, 좌우/상단 이탈은 호밍으로 복귀.

**⚠️ 미검증**: 위 전부 실제 멀티 플레이 최종 검증 필요.
...
### 🗓️ 2026-07-21 세션 (D) — 게임 이탈 시 계속(전체 게임) + 2인 남은사람 승
`GameModule.onPeerLeft` 를 전체 게임에 구현. 이전엔 원카드/블루마블/늑대인간만 있었음.
- **🏗️ 토대(gameScreen.ts)** — (1) 호스트 이탈 게이트를 `remaining >= minPlayers` → **`remaining >= 1`** 로 완화. "계속/남은사람 승 종료"는 각 게임 onPeerLeft 가 자기 종료조건으로 판단 → **2인 게임도 상대 나가면 남은 사람 승**(예전엔 메뉴 튕김). (2) **게스트 측 `player_left` → `gameModule.onPeerLeft?.()` 라우팅 추가** — 로컬시뮬 게임이 게스트에서도 나간 사람 정리(호스트-authoritative 게임은 `!isHost` 가드로 no-op).
- **⚡ 실시간 4종** — 사과·라면(`otherScores` 제거, 타이머 자동종료라 유령 랭킹만 방지) / 배틀테트리스(`opponents` 제거 + `markPlayerOut`=이탈=탈락, 안 하면 last-standing 무한대기) / 반응속도(`opponents`+`finals` 제거 + `expectedPlayerCount--` + 즉시 마감, 안 하면 120초 워치독 대기).
- **🎯 턴제 6종** — 끝말잇기·폭탄끝말(WordChain `alive`/`getNextTurn` 재사용, 1명 남으면 승) / 포트리스(소유 포대 파괴 + 생존 소유자≤1 종료 재현) / 라이어·그림퀴즈·이야기그림(라운드형: 타임아웃 폴백이 진행 보장 → 활성≤1명이면 종료, 그림퀴즈=출제자 이탈 시 즉시 endRound, 이야기그림=나간 좌석 제출됨 처리).
...
**같은 세션 늑대인간 수정**:
- (커밋 652ec0f) 빈 화면 버그 해결(위 "✅ 해결됨" 참고) + deal 흐름을 "각자 역할 확인 후 확인 버튼 → 전원 확인 시 밤"으로(solo 자동진행 제거, 봇은 자동 ready) + 역할 7종 채워진 파스텔 일러스트(`ROLE_SVG`, viewBox 48).
- (커밋 6616b6e) 일러스트가 작게 나오던 것 수정 — `.ic` 안 svg 가 컨테이너를 안 채우던 문제(`.ww-root .ic svg{width:100%;height:100%}`) + 카드 리빌 아이콘 56→88px. **도움말 패널 추가**: 게임 영역 우상단 `?` 버튼 → 진행 순서 + 승패 규칙 + "이 게임의 역할"(세팅이 인원별 고정이라 현재 3/4/5인 구성 정확히 표시, `showHelp()`).

### 🗓️ 2026-07-20~21 세션 — 🆕 한밤의 늑대인간(werewolf) + 브루마블 카드/버그 수정

**🆕 한밤의 늑대인간(werewolf) 신규 게임** `src/games/werewolf/`(rules/netSync/render/index/thumbnail + registry 등록). id `werewolf`, 이름 **한밤의 늑대인간**, 3~5인, host-authoritative P2P. 목업(werewolf-mockup 아티팩트) 먼저 확정 → 구현. **3~5인 핵심 역할부터**(Henry 선택, 6인+ 하수인·비밀결사·가죽장수는 후속).
- **히든롤 룰**: 단 한 번의 밤·낮·투표로 끝. 역할 = 늑대인간(×2)·예언자·강도·말썽쟁이·마을사람·주정뱅이·불면증. 인원별 세팅 = **플레이어수+3장**(3인 6장 / 4인 7장에 주정뱅이 / 5인 8장에 불면증). ※규칙서 "4인=7장" 표기가 나열 카드(8장)와 안 맞아 N+3 공식으로 재조정.
- **카드 3-상태 모델**(핵심): `origRole`(처음 받은 역할=밤 행동 기준, 불변) / `curCard`(교환 반영=승패 기준) / `center` 3장. 강도·말썽쟁이·주정뱅이가 밤에 카드 교환 → 낮엔 팀이 바뀔 수 있음.
- **밤 엔진(호스트)**: 세팅에 든 밤-행동 역할을 순서대로(늑대→예언자→강도→말썽쟁이→주정뱅이→불면증). **카드가 가운데에만 있어도 스텝 진행**(랜덤 2.5~4.5초 대기로 어떤 역할이 없는지 은폐). 정보 받는 역할(늑대/예언자/강도/불면증)=결과 확인 후 진행, 눈감고 하는 역할(말썽쟁이/주정뱅이)=즉시. 비밀은 `ww:role`/`ww:nightInfo` targeted, 공개상태만 `ww:sync`.
- **투표(공식룰)**: 최다 득표 처형(동표 전원), 최다 1표 이하면 아무도 안 죽음. 승패 = 처형자 중 늑대 있으면 시민승 / 시민만 죽으면 늑대승 / 아무도 안 죽으면 플레이어 중 늑대 없을 때만 시민승.
- **낮 토론** = 인게임 채팅(`chatLog` in sync). **결과** = 전원 최종/처음 카드 리빌 + 가운데 3장 공개 + 처형/승패.
- **솔로 프리뷰**: 봇 2명 자동 참가(밤행동/투표 자동), deal에서 역할 카드 2.6초 보여준 뒤 **자동으로 밤 진행**(혼자라 확인 버튼 누를 상대 없음).
- **UI**: 초기 몰입형 다크테마 → "앱이랑 안 맞고 안 보인다" 피드백으로 **밝은 파스텔**로 전환(theme 톤). 밤은 🌙+라벤더 포인트로 표현. 인라인 SVG 역할 아이콘.
- **검증**: TS 미설치라 헤드리스 시뮬 harness 제작(Node 26 + vite 번들 + 관대한 DOM mock + 가상 시계)로 deal→밤→낮→투표→결과→endGame **완주 + 버튼 클릭 배선 + 삼켜진 렌더 예외 0건** 확인. **실제 멀티(3인+) 최종 검증은 아직.**

**🔧 브루마블(blue-marble) 카드/버그 수정** (Henry 실플레이 피드백):
- **인수 후 건설**: 남의 땅 인수 직후에도 지을 수 있으면 건설창 연결(구매와 동일).
- **무인도 탈출 주사위**: 탈출 실패 시에도 스핀/합계 보여준 뒤 턴 넘김(이전엔 sync/render 없이 바로 advanceTurn → 애니·결과 안 보임).
- **황금카드 즉발/보관 확정**: 카드 모달의 무의미한 "보관" 버튼 제거 → 즉발 카드는 전부 즉시 사용. **세계여행권=즉발**(세계여행 칸으로 이동, 건물 X), **올림픽 개최권=즉발**(내 도시 즉시 개최), **출발로 이동=즉발**(월급+건설 O). **보관형은 2종만**: 통행료 면제권 · 무인도 탈출권(무인도일 때만 사용, 보관함 버튼도 그때만 활성).
- **사회복지기금 카드 삭제**: `fund` effect 제거, 카드 **id 0~17 재번호**(인덱스=id 유지). 세금/수수료로 낸 돈은 이제 은행 소멸(state.fund는 dead sink로 남음).
- **파산 시퀀스**: 파산이 이동 애니 중 즉시 종료되던 것 → 말 착지 완료 → 통행료 fx → "파산!" 오버레이 → (텀) 결과화면 순서. `renderer.isBusy()` 폴링으로 애니 끝난 뒤 종료.
- 커밋 2개(werewolf / blue-marble). **push는 Henry가.**

### 🗓️ 2026-07-18~20 세션 — 🆕 브루마블(blue-marble) 신규 게임
새 게임 `src/games/blue-marble/` (rules.ts / netSync.ts / index.ts / render.ts / thumbnail.svg / registry 등록). id `blue-marble`, 이름 **브루마블**, 2~4인, 호스트 authoritative P2P. 모두의마블 월드맵 참조. **다수 반복 리뷰로 확정 — 실제 게임플레이(멀티/솔로) 최종 검증은 아직(미검증).**

- **🗺️ 보드**: 9×9, 총 **32칸**(한 변 8칸, 모서리 포함), 반시계(출발 우하→왼쪽). 모서리=출발/무인도/올림픽/세계여행. 도시 14 + 관광지 5(독도·발리·하와이·푸켓·타히티) + 황금열쇠 3 + 보너스게임 + 국세청. `cell()`=9×9 매핑, `lineOf()`. 라인 그룹색: 초록(green/teal)·파랑(sky/navy)·핑크(pink/rose)·주황빨강(orange/red). 관광지=그라데이션(섬 파랑→흰색 / 해변 붉은→흰색), 이름 중앙정렬, 아이콘 제거.
- **💰 경제**: 시작 300만 / 월급 20만 / 대지 라인별 5~8만·10~15만·18~25만·30~40만 / 건물비 라인별 고정(`LINE_BUILD`, 별장·빌딩·호텔·랜드마크) / 무인도탈출 30만·세계여행 40만·세금 30만·보너스 판돈 10·20·30만.
- **🏙️ 건설**: 별장→빌딩→호텔→랜드마크(kind villa/house2/apt/landmark 유지, name만 변경). **바퀴 잠금 유지**(집 해금에 라운드 필요, Henry가 정함). 구매 즉시 건설창→**체크(다중선택)→완료** 일괄 건설. 랜드마크=인수 불가.
- **💵 통행료 배수(곱연산)**: 컬러 독점 ×2 · 올림픽 개최 ×2~5 · **섬(개수 기반 1개×1·2×2·3×4)** · **해변(방문 기반 ×1~3, 누가 밟든 +1)**. `tollBreakdown`이 항목별 배수 반환(뱃지·상세창 공용). 정전(카드)이면 통행료 0.
- **🏆 승리**: 파산승 + **독점 즉시승**(트리플=컬러3세트 / 라인=한 변 전체 / 관광지=섬 5곳).
- **🎴 특수칸**: 올림픽(내 도시 개최, 팡파레·오륜기 아이콘) / 세계여행(비용→다음턴 원하는 칸 클릭 이동, 비행기 애니) / 보너스게임(할지/판돈 선택→2지선다 ×2누적 최대8배·중간받기) / 출발 정확히 멈춤→추가건설 / 국세청=**이벤트 창(모두에게 표시, 밟은 사람만 확인해 닫음)**.
- **🎁 황금열쇠/포춘카드**: 가중치 뽑기 19종. 즉발(은행이자·보너스·**복권 +150만 희귀 JACKPOT**·병원비·과속·재산세10%·기금독식·출발이동·무인도유배·뒤로3칸·최고가도시) + 보관형(올림픽개최권·통행료면제권·무인도탈출권·세계여행권, 자동보관·자유사용) + **공격형(도시교환·지진=건물1단계파괴·정전=통행료3턴0)**. 각 카드 전용 연출(말날기·파편·스왑글로우·동전샤워·토스트) + reduced-motion 대응.
- **🎨 UI**: 소유 도시=주인색 두꺼운 테두리(내 땅은 6px+흰 내부선, 상대 6px), 플레이어색 진한 채도(파스텔 도시색과 대비), 말=폰 모양 SVG, 배수 뱃지(×N, 노랑→빨강), 주사위 합/더블 표시, 상대 행동중=카드크기 카드(딤X). 결과창=브루마블 전용 자산순위(공용 resultScreen.ts에 `parseBlueMarbleSummary`/`buildBlueMarbleResultHTML` 추가) + 승패 이모지→아이콘.
- **🤖 솔로 프리뷰**: 더미 '연습 상대' 자동 진행(모든 pending 대응). 이동 애니는 **굴린 사람(moverId) 캡처**로 처리(턴 넘어가도 그 말만), 주사위 스핀→이동→결정 순서, 도착 후 텀 두고 모달.
- **⚠️ 알려진 미완성/미검증**:
  - **실제 멀티/솔로 플레이 최종 검증 안 함** (Henry가 나중에 확인).
  - v2 공격카드 대상선택은 됨. **통행료 면제권은 사용 시 다음 통행료 면제 로직 연결됨**.
  - `우주여행`(old) → `세계여행`으로 정리. `fund`(세금 적립)는 사회복지기금 카드로만 회수.
  - **TS 미설치 주의**: `npm run build`(vite/esbuild)는 타입체크 안 함 → import 누락/오타가 런타임에서만 터짐(과거 `canBuild` 미import로 게임 먹통 겪음). 심볼 추가 시 import 수동 확인 필수.

### 🗓️ 2026-07-17 세션 (C) — 배틀 테트리스 인게임 UI 리디자인 + 조작감
- **🧱 테트리스 레이아웃 재구성** (`battle-tetris/render.ts` 전면 재작성) — 목업 반복 리뷰 후 확정.
  - **논리폭 가변**(높이 420 고정): HOLD·필드·NEXT는 고정, 우측 상대 그리드는 살아있는 상대 수만큼만 폭 차지 → **솔로면 우측 숨겨 빈 공간 제거**(AlphaTest 혼자 테스트 시 세로로 딱 붙음). `computeOppLayout(count)`로 매 프레임 `logicalW` 계산해 fitContain에 넘김.
  - **상대 우측 미니 그리드** — 1명=1열 / 2~4명=2열 / 5명+=3열, 셀은 영역 안에서 자동 축소(최대 9명). 닉+OUT 오버레이. 관전자 격자는 별도(SPEC_W 660).
  - **NEXT 직사각형 1개에 다음 2조각** 세로. **HOLD 4×4**. 패널은 반투명 흰 라운드(프로스티드풍).
  - **"지운 줄(LINES)" HUD 제거** — 결과창에서만 보여주면 됨(Henry).
- **⚔️ 공격 시각화** — (1) **받을 가비지 게이지**: 필드 **오른쪽 끝에 딱 붙는** 얇은 세로 바(`pendingGarbage`∝높이, 아래→위). (2) **위험 테두리**: 스택이 상단 3행 닿으면 필드 테두리 빨강+글로우(`stackNearTop`). (3) **방해줄 플래시**: `garbage_injected`/`unbreakable_injected` 이벤트 → `renderer.flashGarbage(count)` → 하단 해당 줄 0.5초 핑크 페이드아웃 + 경계선 강조.
- **🎨 블록 스타일** (`pieces.ts`+`render.ts` drawCell) — 색을 파스텔이되 **또렷하게**(채도↑: I `#5ecdec`/O `#ffcf5c`/T `#a685f5`/L `#ffa863`/J `#7c98ee`/S `#5fdca8`/Z `#ff7ba1`, stroke 진하게). 모서리 **덜 둥글게**(반경 size×0.14), 얇은 여백(0.75)로 칸 분리, 진한 외곽선 + 위 하이라이트만(무거운 베벨/그림자 제거). ※ "무겁다"는 색이 아니라 조작감 얘기였음 → 아래 튜닝.
- **🕹️ 조작감 튜닝** (`index.ts` 상수) — **DAS 150→100ms**(꾹 눌렀을 때 반응↑), **ARR 30→20ms**(좌우 미끄러짐, 여러 번 조정 끝 20 확정), **소프트드롭 15→8ms**(↓키 낙하 2배 빠르게). 엔진 중력(800)/lock delay(500)/lock리셋은 정상이라 안 건드림.
- **⚠️ 미검증**: 실제 멀티(2~10인) 플레이로 상대 그리드/공격연출/조작감 최종 확인 필요.
...
| 3 | **방장 이양** | 🧪 **1차 구현(범위 B) — 미검증** (게임중단→새 방장 대기실. 멀티 실테스트 필요) |
| 4 | 배틀 테트리스 | ✅ 완료 |
| 5 | 결과 화면 다인용 | ✅ 게임별 전용 UI |

**다음 작업** (우선순위):
- 🆕 **브루마블 실제 플레이 검증** (Henry가 나중에 확인) — 솔로(더미)·멀티(2~4인) 실행하며: 통행료 배수/독점승/파산순서, 카드 19종 효과·연출, 세금 이벤트창 동기화, 세계여행/올림픽/보너스 흐름, 소유 테두리 구분, 밸런스(금액·확률·건물비).
- ✅ 🅒 **게임 이탈 시 계속 — 전체 게임 완료** (2026-07-21 세션 D). 모든 게임에 `onPeerLeft` 구현 + 토대 게이트 완화(2인 남은사람 승) + 게스트 라우팅. **남은 것 = 멀티 플레이테스트 검증**뿐. werewolf 는 minP 미만 시 degenerate 진행(필요 시 별도 종료 처리 여지).
- 🧪 **Phase 3 방장 이양 — 1차 구현 완료(범위 B), 미검증** (커밋: peer.ts adoptPeer/promoteToHost/reconnectToNewHost + waitingRoom attemptHostMigration + gameScreen/waitingRoom 게스트 onDisconnect). 흐름: 호스트 이탈 감지 → 공유 `RoomState.players`에서 후계자=players[1] 결정론 선출 → 후계자는 자기 peer 승격(남들이 아는 peerId)해 대기실(호스트), 나머지는 그 peerId로 재연결 → **게임 중단, 전원 새 방장 대기실**. **남은 것**: ①실제 2+클라 테스트(핵심) ②승격 방이 새 게임 시작 시 gameScreen 이 깨진 코드로 공개목록 등록할 수 있음(migrated 플래그 gameScreen 미전달) ③범위 C(게임 상태까지 이관)는 여전히 안 함 ④후계자·다음후계자 모두 끊긴 경우 연쇄선출은 provisional players 로 동작하나 미검증.
- 🅓 **알까기 재설계** — 판 4변 배치라 5인+ 불가 + "먼저 치면 유리" 밸런스. 다른 형식으로 갈아엎기로 함(Henry).
- 🅔 **똥 피하기 공격변형(후속)** — ⭐ 아이템 먹으면 랜덤 상대에게 똥폭탄 투하.
- ~~🅕 남은 저위험 UI (모달 스크롤락 / N인 헤더 2명 표시 / 다트 랭크 여백)~~ — **안 하기로 함(드롭, 2026-07-29 Henry 확인).**
- ✅ **신규 게임 후보: 히든롤(마피아류)** — 원나잇 인랑 → **한밤의 늑대인간**으로 구현 완료. 레지스탕스 → **아발론**으로 구현 완료(세션 F). 둘 다 host-authoritative + 채팅.
