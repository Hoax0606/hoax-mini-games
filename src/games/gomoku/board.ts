 ...
 *   - **장목 금수 (양쪽)**: 두면 6목 이상 연속되는 자리는 흑/백 모두 금수
 *   - **3-3 금수 (흑만)**: 흑이 두면 "열린 3" 이 두 방향 이상 동시에 생기는 수는 금수
 *   - **4-4 금수 (흑만)**: 흑이 두면 "4 라인" 이 두 방향 이상 동시에 생기는 수는 금수
 *   - 5목 완성 수는 모든 금수보다 우선 (둘 수 있고 승리)
 *
 * 좌표계:
 ...
 *
 * 금수 규칙 (캐주얼 렌주룰):
 *   1. 장목 (6목 이상) — 흑/백 모두 금수. 정확히 5목만 승리라서 6목 이상은 무효.
 *   2. 3-3 — **흑만**. 두면 "열린 3" 이 두 방향 이상 생기는 수.
 *      · 열린 3 = 양쪽 끝이 비어있는 3연속 (다음 턴에 열린 4로 발전 가능)
 *   3. 4-4 — **흑만**. 두면 "4 라인" 이 두 방향 이상 생기는 수.
 *      · 4 라인 = 한 방향에 같은 색 4개 연속 (열린 4, 닫힌 4 모두 포함)
 *
 * 예외: 이번 수로 **5목이 완성되면** 모든 금수보다 승리 우선.
 *       (5가 완성되는 방향이 있으면 그 자체로 합법수)
 *
 * 약식 판정:
 *   엄밀한 렌주룰은 "열린 3" / "4" 를 재귀적으로 확인하지만,
 *   여기선 단순 "연속 + openEnds" 규칙으로 충분 — 친구끼리 게임 수준.
 */
export function isLegal(
  ...
  // 방향별 분석 수집
  let threeCount = 0; // 열린 3 (openEnds=2, len=3) — 흑만 사용
  let fourCount = 0;  // 4 라인 (len=4, openEnds 무관) — 흑만 사용
  let wouldWin = false;
  ...
  for (const { dx, dy } of DIRS) {
    ...
    // 흑만 적용되는 3-3 / 4-4 판정 자료 수집
    if (stone === 'B') {
      if (len === 4) fourCount++;
      if (len === 3 && openEnds === 2) threeCount++;
    }
  }

  // 승리 수는 흑의 3-3 / 4-4 금수 스킵 (장목은 위에서 이미 false)
